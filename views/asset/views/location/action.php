...
if ($method === "POST" && $action === "create") :
  try {
    ...
    $Location->create([$name, $login__id]);

    $Validation->alert("success", "เพิ่มข้อมูลเรียบร้อยแล้ว", "/asset/location");
...
if ($method === "POST" && $action === "update") :
  try {
    ...
    $Location->update([$name, $login__id, $status, $id]);

    $Validation->alert("success", "ดำเนินการเรียบร้อยแล้ว", "/asset/location/view/{$id}");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
endif;
